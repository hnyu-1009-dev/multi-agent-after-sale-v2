...
async def resolve_user_location_from_text(
    ...
    if user_input in RELATIVE_LOCATIONS:
        logger.info(
            f"[Location] Detected relative term '{user_input}', forcing IP location fallback."
        )
        user_input = ""
    ...
    if user_input:
        try:
            ...
            geo_result = await baidu_mcp_client.call_tool(
                tool_name="map_geocode", arguments={"address": user_input}
            )
            ...
            if (
                ...
                and "lat" in result["location"]
                and "lng" in result["location"]
            ):
                ...
                return json.dumps(
                    {"ok": True, "lat": lat, "lng": lng, "source": "geocode"},
                    ensure_ascii=False,
                )
            else:
                logger.warning(
                    f"[Location] Geocode returned invalid result: {geo_result}"
                )
        except Exception as e:
    ...
    if user_ip and user_ip not in ("127.0.0.1", "localhost", "::1"):
        try:
            ...
            if data.get("status") != 0:
                logger.warning(
                    f"[Location] IP location API error: {data.get('message', 'unknown')}"
                )
            ...
            return json.dumps(
                {"ok": True, "lat": lat, "lng": lng, "source": "ip"}, ensure_ascii=False
            )
