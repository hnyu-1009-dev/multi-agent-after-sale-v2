...
main_model_client = AsyncOpenAI(
    base_url=SF_BASE_URL, api_key=SF_API_KEY  # 硅基流动base url  # 硅基流动api key
)
# 子模型客户端(干活的子Agent使用)
...
main_model = OpenAIChatCompletionsModel(
    model=MAIN_MODEL_NAME, openai_client=main_model_client
)

# 创建子调度模型
sub_model = OpenAIChatCompletionsModel(
    model=SUB_MODEL_NAME, openai_client=sub_model_client
)
