...
class IngestionProcessor:
    ...
    def ingest_file(self, md_path: str) -> int:
        ...
        # ②. 用这4个问题去分别检索（多路召回） 1个query:4:answer  4个query:16:answer ---16个文档块（去重）--->10个（1:二次利用模型对检索到大量内容的chunk做降噪（1.只提取 2.总结）2:.利用嵌入模型二次优化chunk）--->留4个--->LLM(1.稳 2.精准)
        ...
        for doc in documents:
            if (
                len(doc.page_content) < 3000
            ):  # 评估一下小文件的内容长度（获取一个平均值）
                # a.不用切分
...
if __name__ == "__main__":
    ...
    ingest_processor.ingest_file(
        "D:\\WorkSystem\\multi-agent-after-sale\\backend\\knowledge\\data\\crawl\\0430-联想手机K900常见问题汇总.md"
    )
